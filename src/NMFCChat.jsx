...
{ field: 'volume\_per\_unit', question: 'What is the size or volume of each bottle?', options: \['0.5L', '1L', '2L', '5L'] },
{ field: 'filled', question: 'Are they empty or filled?', options: \['empty', 'filled'] },
{ field: 'contents', question: 'If filled, what is the content?', options: \['water', 'juice', 'chemical', 'empty'], conditional\_on: { filled: 'filled' } },
...
{ field: 'total\_weight', question: 'What is the total weight of the shipment?', options: \['<50 lbs', '50-100 lbs', '100-200 lbs', '200+ lbs'] },
...
const shouldSkip = current?.conditional\_on &&
Object.entries(current.conditional\_on).some((\[key, val]) => answers\[key] !== val);
...
setLoading(true);
try {
const res = await fetch("[https://nmfc-api-backend.railway.app/classify](https://nmfc-api-backend.railway.app/classify)", {
method: "POST",
...
\<h2 style={{ fontWeight: 'bold' }}>✅ Summary</h2> <pre>{JSON.stringify(answers, null, 2)}</pre>
{loading && <p>🔄 Classifying with GPT + FAISS...</p>}
{result && !loading && (
\<div style={{ marginTop: '1rem', padding: '1rem', background: '#e0ffe0' }}> <h3>📦 NMFC Classification Result</h3>
{result.error ? (
...
return (
\<div style={{ marginTop: '2rem' }}> <p><strong>{current.question}</strong></p>
